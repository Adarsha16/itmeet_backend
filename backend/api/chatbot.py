...
from backend.configs.llm_Context import EVENT_CONTEXT
from backend.configs.environment import GROQ_API_KEY
...
def check_rate_limit(session_id: str):
    ...
    session_requests[session_id] = [
        t for t in session_requests[session_id] if now - t < WINDOW_SEC
    ]
...
def cleanup_inactive_sessions():
    ...
    inactive = [
        sid
        for sid, last_active in session_last_active.items()
...
async def chat_bot(req: ChatRequest):
    ...
    # api call -> loop -> random string -> accept
    rate_limit_details = check_rate_limit(req.session_id)

    if req.session_id not in chat_history:
        chat_history[req.session_id] = [{"role": "system", "content": EVENT_CONTEXT}]
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        return {"reply": reply, "rate_limit": rate_limit_details}
